...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  return {
    title: t('europeanFirmsTitle'),
    description: t('europeanFirmsDescription'),
  };
}

export default function ForEuropeanFirmsPage({ params: { locale } }: { params: { locale: string } }) {
  setRequestLocale(locale);
  return <ForEuropeanFirmsContent />;
}

function ForEuropeanFirmsContent() {
  const t = useTranslations('europeanFirms');
      ...
      <section className="bg-mirai-off-white section-padding">
        <div className="max-w-content mx-auto px-6 md:px-12 lg:px-8">
          <div className="max-w-2xl">
            <h2 className="font-display text-subsection mb-6">{t('whyMiraiTitle')}</h2>
            <p className="text-body text-mirai-gray-dark leading-relaxed">{t('whyMiraiBody')}</p>
          </div>
        </div>
      </section>
